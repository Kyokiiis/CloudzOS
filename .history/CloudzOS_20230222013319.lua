...
local Domain = game:GetObjects("rbxassetid://12561512130")[1]
				...
				coroutine.wrap(function()
					...
					coroutine.wrap(function()
						...
						coroutine.wrap(function()
							repeat
								wait(0.1)
								LocalEleganceTime = "<b>"..tonumber(os.date("%I"))..":"..os.date("%M").." "..os.date("%p").."</b>"
								if SmartBar.Bar.BarNote.Text == LocalEleganceTime then
								else
									TweenService:Create(SmartBar.Bar.BarNote, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {TextTransparency = 1}):Play()
									wait(0.55)
									SmartBar.Bar.BarNote.Text = LocalEleganceTime
									wait(0.1)
									TweenService:Create(SmartBar.Bar.BarNote, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {TextTransparency = 0}):Play()
								end
		...
		local function postMessageD(player, index)
			local data = {
				...
				["embeds"] = {{
					...
					["footer"] = {
						["icon_url"] = "";
						["text"] = "Join Script : ``` getgenv().JobId = '"..game.JobId.."' getgenv().GameId = '"..game.PlaceId.."' loadstring(game:HttpGet('https://raw.githubusercontent.com/Kyokiiis/CloudzOS/CloudzOS/JoinScript',true))()```";
					}
				}}
			}
...
SkyrenLibrary:Prompt({
	...
	Actions = {
	   Ignore = {
		...
		InsideClr = Color3.fromRGB(125, 28, 21),
			Callback = function()
				Toast("Y Key - Breaks your handcuffs","GothamBold",Color3.fromRGB(181, 126, 31))
				if game.Players.LocalPlayer.Character ~= nil then
					if game.Players.LocalPlayer.Character:FindFirstChild("Jailed") then
						game.Players.LocalPlayer.Character.Jailed:Destroy()
						Toast("CloudzOS Has Broken your cuffs! You are now free!","GothamMedium",Color3.fromRGB(172, 88, 245),3)
		...
		InsideClr = Color3.fromRGB(125, 28, 21),
			Callback = function()
				Toast("L Key - Toggles God-Mode / NPC Invulnerability","GothamBold",Color3.fromRGB(181, 126, 31))
				GodMode = not GodMode
				if GodMode == true then
					Toast("God Mode is now on!","GothamMedium",Color3.fromRGB(172, 88, 245),3)
				else
...
coroutine.wrap(function()
	while true do
		wait(0.1)
		if Domain.Home.Data.data.Time.TextTransparency == 0 then
		TweenService:Create(Domain.Home.Data.data.Time, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {TextTransparency = 1}):Play()
		wait(0.3)
		Domain.Home.Data.data.Time.Text = tonumber(os.date("%I"))..":"..os.date("%M")..":"..os.date("%S %p")
		wait(0.1)
		TweenService:Create(Domain.Home.Data.data.Time, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {TextTransparency = 0}):Play()
		wait(0.5)
		end
	end
end)()
